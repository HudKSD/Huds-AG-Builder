...
class NL2ESAgent:
    ...
    async def _resolve_index_from_hint_by_listing(self, hint: str, trace: List[Dict[str, Any]]) -> Optional[str]:
        ...
        for pat in allowed:
            res = await self.tools.run("list_indices", {"pattern": pat, "max_results": 300})
            self._trace_tool_call(
                trace,
                tool_id="list_indices",
                params={"pattern": pat, "max_results": 300},
                result=res,
                progression=["Identifying the most relevant data source"],
            )
            if isinstance(res, dict):
                for idx in (res.get("indices") or []):
                    if isinstance(idx, str):
                        all_indices.append(idx)

        if not all_indices:
            self._trace_reasoning(trace, "No concrete indices discovered from allowlist; fallback to pattern scoring.")
            return None

        ranked = sorted(set(all_indices), key=lambda n: (-self._score_index_name(hint, n), len(n)))
        best = ranked[0] if ranked and self._score_index_name(hint, ranked[0]) > 0 else None
        if best:
